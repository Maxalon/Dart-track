...
import com.dartrack.model.BASEBALL_MAX_DARTS
import com.dartrack.model.BaseballState
import com.dartrack.viewmodel.GameViewModel

/**
 * Baseball entry screen, built to the app's single-screen layout convention:
 * everything fits on one screen (no verticalScroll). A compact header shows the
 * current inning number, a weighted players area highlights the active thrower
...
fun BaseballScreen(
    ...
    val state = record?.state as? BaseballState ?: return
    ...
    val remaining = BASEBALL_MAX_DARTS - entered
    ...
    Column(
        ...
        // ---- Compact header: current inning + "to throw". ----
        Row(
            modifier = Modifier
                .fillMaxWidth()
            ...
            Column(modifier = Modifier.weight(1f)) {
                if (state.isFinished) {
                    ...
                    Text(
                        "3 darts at $target · runs = S + 2×D + 3×T",
                        ...
                        Column(modifier = Modifier.weight(1f)) {
                            ...
                            Text(
                                when {
                                    ...
                                    else -> "inning ${state.currentTarget(idx)} · ${ps.darts} darts"
                                },
        ...
        Column(
            ...
            if (state.isFinished) {
                ...
                Row(
                    ...
                    Button(
                        onClick = {
                            vm.applyBaseballTurn(singles, doubles, triples)
                            reset()
                        },
