...
@Composable
fun DartTrackTheme(
    // [themeMode] decides dark vs light: SYSTEM follows the OS, LIGHT/DARK force
    // it. Defaulting to SYSTEM keeps existing call sites (no args) behaving as
    // before (isSystemInDarkTheme()). [dynamicColor] gates Material You exactly
    // as before, still also gated on Android 12+ (S).
